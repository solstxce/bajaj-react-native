...
export function FormModal({ visible, onClose }: Props) {
  ...
  const [taskBranch, setTaskBranch] = useState(branches[0]?.id || 1);
  ...
  const resetForms = () => {
    ...
    setTaskTitle(""); setTaskAudience("lc"); setTaskBranch(branches[0]?.id || 1); setTaskSchedule("Daily"); setTaskZone(""); setTaskDeadline(""); setTaskPriority("High"); setTaskProofRule("true"); setTaskNotes("");
    ...
    setUserName(""); setUserRole("lc"); setUserBranch(branches[0]?.id || 1);
    setErrors({});
  };
  ...
  const forms: Record<string, { title: string; subtitle: string; render: () => React.ReactNode }> = {
        ...
        return (
          <View style={{ gap: spacing.lg }}>
            ...
            <View>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
                {branches.map((b) => (
                  <TouchableOpacity key={b.id} onPress={() => setTaskBranch(b.id)} style={{ paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: borderRadius.full, backgroundColor: taskBranch === b.id ? colors.brand : colors.slate50 }}>
                    <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: taskBranch === b.id ? colors.white : colors.textSecondary }}>{b.name.split(" ")[0]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
    ...
    appliance: {
      ...
      render: () => {
        const handleSubmit = () => {
          const result = validate(applianceSchema, { name: applianceName, category: applianceCategory, zone: applianceZone, brand: applianceBrand, model: applianceModel });
          if (!result.success) { setErrors(result.errors); return; }
          createAppliance({ name: applianceName, category: applianceCategory, zone: applianceZone, brand: applianceBrand, model: applianceModel });
          handleClose();
        };
        return (
          <View style={{ gap: spacing.lg }}>
            ...
            <View style={{ flexDirection: "row", gap: spacing.lg }}>
              ...
              <View style={{ flex: 1 }}>{input("Zone", applianceZone, setApplianceZone, undefined, "zone")}</View>
            </View>
            <View style={{ flexDirection: "row", gap: spacing.lg }}>
            ...
            <View>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
                {branches.map((b) => (
                  ...
                  >
                    <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: visitBranch === b.id ? colors.white : colors.textSecondary }}>{b.name.split(" ")[0]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              {showError("branchId")}
        ...
        return (
          <View style={{ gap: spacing.lg }}>
            ...
            <View>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
                {branches.map((b) => (
                  <TouchableOpacity key={b.id} onPress={() => setUserBranch(b.id)} style={{ paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: borderRadius.full, backgroundColor: userBranch === b.id ? colors.brand : colors.slate50 }}>
                    <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: userBranch === b.id ? colors.white : colors.textSecondary }}>{b.name.split(" ")[0]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
