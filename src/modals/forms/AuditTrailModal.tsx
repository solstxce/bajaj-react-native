...
import { X, History, Activity, FileText, CheckCircle, AlertTriangle, RefreshCw } from "lucide-react-native";
...
const auditEntries = [
  { time: "11:01", text: "Worker task 101 remains pending and still needs image proof.", icon: Activity, color: colors.warning },
  { time: "10:18", text: "Expense request 402 routed to manager queue.", icon: FileText, color: colors.brandSecondary },
  { time: "10:02", text: "Worker task 104 revoked with corrected voucher note.", icon: RefreshCw, color: colors.error },
        ...
        <Animated.View style={{ backgroundColor: colors.card, borderRadius: borderRadius["6xl"], width: "100%", maxWidth: 420, maxHeight: "85%", opacity, transform: [{ translateY }], ...shadows.modal }}>
          <TouchableOpacity activeOpacity={1} onPress={() => {}}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: spacing.xl, paddingTop: spacing.xl }}>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: fontSize.lg, fontWeight: "700", color: colors.text }}>Audit trail</Text>
                <Text style={{ fontSize: fontSize.xs, color: colors.textSecondary, marginTop: spacing.xs }}>Recent system actions, approvals and proof-based movements</Text>
              </View>
            ...
            <ScrollView style={{ maxHeight: 400 }} contentContainerStyle={{ padding: spacing.xl, gap: spacing.md }}>
              {auditEntries.map((entry, i) => (
                <View key={i} style={{ flexDirection: "row", gap: spacing.lg, backgroundColor: colors.slate50, borderRadius: borderRadius.xl, padding: spacing.xl }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: fontSize.sm, fontWeight: "600", color: colors.text }}>{entry.time}</Text>
