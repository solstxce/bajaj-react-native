...
export function BranchDetailScreen({ branchId, onBack }: Props) {
  ...
  const branchUsers = scopedUsers.filter((u) => u.branchId === branchId);
  const branchTasks = scopedTasks.filter((t) => t.branchId === branchId);
  const branchComplaints = scopedComplaints.filter((c) => c.branchId === branchId);
  const branchAppliances = scopedAppliances.filter((a) => a.branchId === branchId);
    ...
    <ScreenWrapper>
      ...
      <View style={{ gap: spacing.xl }}>
        ...
        <Card variant="glass">
          ...
          <View style={{ gap: spacing.md }}>
            {branchAppliances.slice(0, 4).map((app) => (
