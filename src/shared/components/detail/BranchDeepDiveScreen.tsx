...
export function BranchDeepDiveScreen({ branch, onBack }: Props) {
  ...
  const branchComplaints = scopedComplaints.filter((c) => c.branchId === branch.id);
  const branchAppliances = scopedAppliances.filter((a) => a.branchId === branch.id);
  const branchAttendance = scopedAttendance.filter((a) => {
  ...
  const employees = branchUsers.filter((u) => u.role !== "worker" && u.role !== "rm" && u.role !== "branchManager");
    ...
    return (
      <View style={{ gap: spacing.md }}>
        ...
        {branchAppliances.length === 0 ? (
          ...
          <View key={app.id} style={{ backgroundColor: colors.card, borderRadius: borderRadius["2xl"], padding: spacing.xl, borderWidth: 1, borderColor: colors.border }}>
