...
export function AlertStrip({ onReviewAlerts, onOpenAudit }: Props) {
  ...
  return (
    <View style={{ backgroundColor: colors.card, borderRadius: borderRadius["5xl"], borderWidth: 1, borderColor: colors.border, padding: spacing.xl }}>
      <View style={{ flexDirection: "column", gap: spacing.lg }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.lg }}>
          <View style={{ width: 40, height: 40, borderRadius: borderRadius.lg, backgroundColor: colors.rose200, alignItems: "center", justifyContent: "center" }}>
            <TriangleAlert size={18} color={colors.rose600} strokeWidth={2} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: colors.text }}>
        ...
        <View style={{ flexDirection: "row", gap: spacing.sm }}>
          {onReviewAlerts ? (
            <TouchableOpacity onPress={onReviewAlerts} style={{ backgroundColor: colors.text, borderRadius: borderRadius["2xl"], paddingHorizontal: spacing.xl, paddingVertical: spacing.md }}>
              <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: colors.white }}>Review alerts</Text>
            </TouchableOpacity>
          ) : null}
          {onOpenAudit ? (
            <TouchableOpacity onPress={onOpenAudit} style={{ backgroundColor: colors.card, borderRadius: borderRadius["2xl"], paddingHorizontal: spacing.xl, paddingVertical: spacing.md, borderWidth: 1, borderColor: colors.border }}>
              <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: colors.textSecondary }}>Open audit trail</Text>
            </TouchableOpacity>
          ) : null}
        </View>
      </View>
