...
import { Mail, Phone, MapPin, Briefcase, Shield, Award, Star, Clock, Building, Users, ChevronRight, Smartphone, CalendarDays, FileText } from "lucide-react-native";
...
export function RmProfileScreen() {
  ...
  return (
    <ScreenWrapper>
      <SectionHeader title="Profile & Authority" action={<QuickButton label="Edit" onPress={() => showToast("Edit profile")} />} />
      ...
      <Card variant="soft" style={{ backgroundColor: colors.text, marginTop: spacing.xl }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.xl }}>
          <View style={{ width: 72, height: 72, borderRadius: 36, backgroundColor: colors.brand, alignItems: "center", justifyContent: "center" }}>
            <Text style={{ fontSize: fontSize["4xl"], fontWeight: "800", color: colors.white }}>{initials}</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: fontSize["3xl"], fontWeight: "800", color: colors.white }}>{currentUser.name}</Text>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.lg, marginTop: spacing.xl }}>
        <View style={{ flex: 1, minWidth: 80, backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.lg, borderWidth: 1, borderColor: colors.border, alignItems: "center" }}>
          ...
          <Text style={{ fontSize: fontSize.xl, fontWeight: "700", color: colors.text, marginTop: spacing.sm }}>{currentUser.rating}</Text>
        ...
        <View style={{ flex: 1, minWidth: 80, backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.lg, borderWidth: 1, borderColor: colors.border, alignItems: "center" }}>
          ...
          <Text style={{ fontSize: fontSize.xl, fontWeight: "700", color: colors.text, marginTop: spacing.sm }}>{branches.length}</Text>
          <Text style={{ fontSize: fontSize.xs, color: colors.textSecondary }}>Branches</Text>
        </View>
        <View style={{ flex: 1, minWidth: 80, backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.lg, borderWidth: 1, borderColor: colors.border, alignItems: "center" }}>
          <Star size={18} color={colors.warning} strokeWidth={2} />
          <Text style={{ fontSize: fontSize.xl, fontWeight: "700", color: colors.text, marginTop: spacing.sm }}>{currentUser.tasksClosed}</Text>
        ...
        <View style={{ flex: 1, minWidth: 80, backgroundColor: colors.card, borderRadius: borderRadius.xl, padding: spacing.lg, borderWidth: 1, borderColor: colors.border, alignItems: "center" }}>
          ...
          <Text style={{ fontSize: fontSize.xl, fontWeight: "700", color: colors.text, marginTop: spacing.sm }}>{currentUser.attendancePct}%</Text>
      ...
      <Card variant="glass" style={{ marginTop: spacing.xl }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.md, marginBottom: spacing.lg }}>
          ...
          <Text style={{ fontSize: fontSize.lg, fontWeight: "700", color: colors.text }}>Skills & Expertise</Text>
        </View>
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
          {currentUser.skills.map((skill, i) => (
            <View key={i} style={{ backgroundColor: colors.brandLight, borderRadius: borderRadius.full, paddingHorizontal: spacing.xl, paddingVertical: spacing.sm }}>
              <Text style={{ fontSize: fontSize.sm, fontWeight: "600", color: colors.brand }}>{skill}</Text>
            </View>
          ))}
        </View>
      </Card>
      ...
      <Card variant="glass" style={{ marginTop: spacing.xl }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.md, marginBottom: spacing.lg }}>
          ...
          <Text style={{ fontSize: fontSize.lg, fontWeight: "700", color: colors.text }}>Regional Authority</Text>
        </View>
        <View style={{ gap: spacing.md }}>
          {[
            ...
            <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: spacing.lg, backgroundColor: colors.slate50, borderRadius: borderRadius.lg, padding: spacing.xl }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: fontSize.sm, fontWeight: "700", color: colors.text }}>{item.title}</Text>
