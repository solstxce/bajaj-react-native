...
import { CheckCircle2, Clock, XCircle, CalendarDays, Users, ArrowUpRight } from "lucide-react-native";
...
export function AmAttendanceScreen() {
  const { scopedUsers, scopedAttendance, state, getBranch, currentUser, openUserDetail, showToast } = useApp();
  ...
  const teamUsers = scopedUsers.filter((u) => u.id !== currentUser.id && u.role === "worker");
  ...
  return (
    <ScreenWrapper>
      <SectionHeader
        ...
        action={<QuickButton label="Escalate late cases" icon={ArrowUpRight} onPress={() => showToast("Late cases escalated to concerned staff")} />}
      />
        ...
        <Card variant="glass">
          ...
          <View style={{ gap: spacing.md }}>
            {teamUsers.map((user) => {
              ...
              return (
                <TouchableOpacity key={user.id} onPress={() => openUserDetail(user.id)} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: colors.slate50, borderRadius: borderRadius.lg, paddingHorizontal: spacing.xl, paddingVertical: spacing.md }}>
