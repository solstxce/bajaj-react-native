...
import { CheckCircle2, Clock, XCircle, CalendarDays, Plus, Trash2, Send, ListChecks } from "lucide-react-native";
...
export function LcAttendanceScreen() {
  ...
  const isPresent = myToday?.status === "Present";
  ...
  const handleMarkAttendance = () => {
    ...
    if (validTasks.length === 0) return showToast("Add at least one task for the week");
    ...
    return (
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: spacing.xl, paddingBottom: 40 }}>
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.lg }}>
          <View style={{ flex: 1, minWidth: 280 }}><StatCard label="Present" value={String(todayAttendance.filter((a) => a.status === "Present").length)} meta="Verified geo attendance" accent={colors.success} icon={CheckCircle2} /></View>
          <View style={{ flex: 1, minWidth: 280 }}><StatCard label="Late" value={String(todayAttendance.filter((a) => a.status === "Late").length)} meta="Marked after shift start" accent={colors.warning} icon={Clock} /></View>
          <View style={{ flex: 1, minWidth: 280 }}><StatCard label="Absent" value={String(todayAttendance.filter((a) => a.status === "Absent").length)} meta="No punch today" accent={colors.error} icon={XCircle} /></View>
        </View>
          ...
          {isPresent ? (
            ...
            <View style={{ gap: spacing.xl }}>
              <Card variant="soft" style={{ backgroundColor: colors.text }}>
                <Text style={{ fontSize: fontSize.xs, fontWeight: "400", color: colors.slate300, textTransform: "uppercase", marginBottom: spacing.md }}>Weekly task plan</Text>
                <Text style={{ fontSize: fontSize.sm, color: colors.slate300, marginBottom: spacing.xl }}>List the tasks you plan to complete this week</Text>
              ...
              <TouchableOpacity onPress={handleMarkAttendance} style={{ backgroundColor: colors.brand, borderRadius: borderRadius.lg, paddingHorizontal: spacing.xl, paddingVertical: spacing.md, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: spacing.sm }}>
                ...
                <Text style={{ fontSize: fontSize.sm, fontWeight: "400", color: colors.white }}>Mark attendance with task plan</Text>
              </TouchableOpacity>
            </View>
          )}
        </Card>
  ...
  function renderHistory() {
    ...
    // Generate dates for current month (mocking a few days backwards from '2026-04-26' down to '2026-04-01')
    const calendarDays = Array.from({ length: 26 }, (_, i) => {
      const day = 26 - i;
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: spacing.xl, paddingBottom: 40 }}>
        <Card variant="glass">
          <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.md, marginBottom: spacing.lg }}>
            ...
            <Text style={{ fontSize: fontSize.lg, fontWeight: "400", color: colors.text }}>My Calendar - April 2026</Text>
          </View>
          <View style={{ gap: spacing.lg }}>
            {calendarDays.map((date) => {
              ...
              const completedOnDate = myTasks.filter((t) => (t.id % 26) + 1 === parseInt(date.slice(-2)));
